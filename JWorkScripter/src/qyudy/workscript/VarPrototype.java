...
enum VarPrototype {
	CLEAR {
		...
		String format(ScriptEnvironment environment, String... strs) {
			...
			String title = Utils.getFromList(strs, 0), defaultText = Utils.getFromList(strs, 0);
			String input = environment.openInput(title, defaultText);
			return input;
		}
	...
	static VarPrototype forName(String name) {
		...
		VarPrototype prop = VarPrototype.valueOf(name.toUpperCase());
